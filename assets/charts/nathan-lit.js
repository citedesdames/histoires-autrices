$.get('/assets/data/data1.csv', function (csvString) {
    ...
    for (var i = 0; i < size; i++) {
        if (data1[i].Genre == "F" && data1[i]['Jeu de données'] == "Manuel Nathan - Littérature - Textes et documents" ) {
            autrice++;
            autrice_noms.push(data1[i]["Auteur ou autrice"]);
        } else if (data1[i].Genre == "M" && data1[i]['Jeu de données'] == "Manuel Nathan - Littérature - Textes et documents" ) {
            auteur++;
        }       
    }
    // console.log(autrice);
